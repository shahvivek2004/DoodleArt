...
import { Instruction } from "@/components/Canvas/Instruction";
import { CreateRoomModal } from "@/components/Dashboard/PopupComponent/CreateRoomModal";
import { ShareRoomModal } from "@/components/Dashboard/PopupComponent/ShareRoomModal";
import timeAgo from "@/components/Dashboard/TimeComponent/main";
...
export default function DashBoard() {
  ...
  const projects = useMemo(() => {
    ...
    switch (activeTab) {
      case "recent":
        filtered = data.filter((p) => !p.isInTrash).slice(-5);
        break;
      case "starred":
        filtered = data.filter((p) => p.isFavourite && !p.isInTrash);
        break;
      case "alldraw":
        filtered = data.filter((p) => !p.isInTrash);
        break;
      case "trash":
        filtered = data.filter((p) => p.isInTrash);
        break;
  ...
  return (
    <div className="bg-[#121212] h-screen flex flex-col overflow-hidden ">
      {/* Nav-bar */}
      <nav className="shadow-sm shrink-0">
      ...
      <div className="flex-1 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-full">
          <div className="flex flex-col md:flex-row mx-auto h-full">
            {/* Left sidebar */}
            <div className="w-full md:w-64 mb-6 md:mb-0 md:mr-8 shrink-0">
